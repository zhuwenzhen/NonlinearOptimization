...
\frametitle{A Fisher Iris Example}
    \begin{figure}[htbp] \centering \subfloat[??]{    \label{fig:subfig_a}    \includegraphics{setosa}    }\caption{????} \end{figure}
...
\[y(x)=\frac{1}{1+e^{-x}}\]
%\includegraphics[width=3cm]{1d-sigmoid}
\begin{figure}[t]
\includegraphics[width=6cm]{1d-sigmoid}
\centering
\end{figure}
\end{frame}
%------------------------------------------------
\begin{frame}
\frametitle{Sigmoid Function 2D}
\[y(\mathbf{x})=\frac{1}{1+e^{- ({w_0} + {w_1}  {x_1} + {w_2} {x_2} )  }}\]
%\includegraphics[width=3cm]{1d-sigmoid}
\begin{figure}[t]
\includegraphics[width=6cm]{1d-sigmoid}
\centering
...
\section{Predict binary output from input variables}
Examples:
\begin{itemize}
...
Output 0 or 1 based on decision boundary on linear function:
\begin{equation}
y = \text{sign} (\theta^T x)
\end{equation}
\end{frame}
